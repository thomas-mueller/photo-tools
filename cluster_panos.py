...
def main():
	...
	for index, cluster_index in enumerate(cluster_indices):
		clustered_jpg_files.setdefault(cluster_index, []).append(portrait_jpg_files_dates[index])
	clustered_jpg_files_dates = [cluster for cluster in clustered_jpg_files_dates.values() if len(cluster) > 1]
	...
	for index, jpg_files_dates_cluster in enumerate(progressiterator.ProgressIterator(clustered_jpg_files_dates, description="Move candidates of panoramic photos to subdirectories")):
		raw_files_dates_cluster = [raw_files_dates[raw_files_dates.index(jpg_file_date)] for jpg_file_date in in jpg_files_dates_cluster if jpg_file_date in raw_files_dates]
		...
		if (len(raw_files_dates_cluster) > 0) and (not os.path.exists(raw_sub_dir)):
